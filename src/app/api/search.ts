...
export const searchAction = async (data: { text: string, genres: IGenre }): Promise<IData> => {
  ...
  const request = await fetch(`https://kitsu.io/api/edge/anime?${genresText ? reqUrlGenres + '&' + reqUrlText : reqUrlText}`)
